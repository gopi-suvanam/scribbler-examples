...
async function  getWebStableDiffusion(initProgressCallback,canvas){
		...
		const   wasmSource=await wasmSourceResponse.arrayBuffer();
		...
		const schedulerConst = [];
		for (let i = 0; i < config.schedulerConstUrl.length; ++i) {
		  const schedulerConstReponse  = await fetch(config.schedulerConstUrl[i]);
		  const schedulerConstData=await schedulerConstReponse.json();
		  schedulerConst.push(schedulerConstData)
		}
